    ...
    class Entry {
        constructor(number) {
            ...
            sectionBehavior() {
                ...
                document.querySelector(`#poster${this.number}`).addEventListener('click', Narrow()) //https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener
            }

            Narrow() {
                document.querySelector('#chosenTitle').innerText = data.data[this.number-1].title
                document.querySelector('#chosenPoster').src = data.data[this.number-1].images.jpg.large_image_url

                if (data.data[this.number-1].synopsis == null) {
                    document.querySelector('h3').innerText = "Synopsis to be written."
                }
            
                else {
                document.querySelector('h3').innerText = data.data[this.number].synopsis
                }
            } 
        }
